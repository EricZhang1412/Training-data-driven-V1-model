...
class BillehColumnTorch(nn.Module):
    def __init__(
        ...
        self.gauss_std = torch.tensor(float(gauss_std), dtype=torch.float32, device=self.device)
        self.dampening_factor = torch.tensor(float(dampening_factor), dtype=torch.float32, device=self.device)
        ...
        self.rec_indices = rec_sparse.indices()
        self.rec_shape = rec_shape

        rec_values = rec_sparse.values()
        self.rec_sign = torch.sign(rec_values)
        ...
        self.in_indices = in_sparse.indices()
        self.in_shape = in_shape

        in_values = in_sparse.values()
        self.in_sign = torch.sign(in_values)
    ...
    def recurrent_sparse(self):
        ...
        return torch.sparse_coo_tensor(
            self.rec_indices, vals, self.rec_shape, device=self.device, check_invariants=False
        ).coalesce()

    def input_sparse(self):
        vals = self.constrained_values(self.input_weight_values, self.in_sign)
        return torch.sparse_coo_tensor(
            self.in_indices, vals, self.in_shape, device=self.device, check_invariants=False
        ).coalesce()

    def zero_state(self, batch_size):
        B, N, R, D = batch_size, self.n_neurons, self.n_receptors, self.max_delay
        return (
            torch.zeros(B, N * D, device=self.device),
            self.v_reset[None, :].repeat(B, 1),
            torch.zeros(B, N, device=self.device),
            torch.zeros(B, N, device=self.device),
            torch.zeros(B, N, device=self.device),
            torch.zeros(B, N * R, device=self.device),
            torch.zeros(B, N * R, device=self.device),
        )
    ...
    def forward(self, x, state=None, input_is_current=False):
        ...
        for t in range(T):
            ...
            rest = (torch.rand(B, 10, device=self.device) < 0.1).float().sum(dim=-1)
